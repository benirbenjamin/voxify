...
export function ChoirHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-[#E6F2FC] bg-[#FCFEFF]/90 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-3 sm:px-6 lg:px-8">
        
        {/* Brand Logo & Choir Switcher */}
        <div className="flex items-center gap-2 sm:gap-6">
          <Link href="/dashboard" className="flex items-center gap-2 shrink-0">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#DFF1FF] border border-[#B9E2FF] shadow-sm">
              <Music2 className="h-5 w-5 text-purple-600" />
            </div>
            <span className="text-base sm:text-lg font-extrabold tracking-tight text-[#475569] hidden xs:inline-block">
              Voxify<span className="text-purple-600">Space</span>
            </span>
          </Link>
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 sm:gap-2 rounded-xl bg-[#F5FAFF] px-2.5 sm:px-3.5 py-1.5 text-xs font-bold text-[#475569] border border-[#E6F2FC] hover:bg-[#DFF1FF] transition-all max-w-[130px] sm:max-w-[180px]"
            >
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shrink-0" />
              <span className="truncate">{activeChoir?.name || 'Select Choir'}</span>
              <ChevronDown className="h-3.5 w-3.5 text-[#A8B5C2] shrink-0" />
            </button>
